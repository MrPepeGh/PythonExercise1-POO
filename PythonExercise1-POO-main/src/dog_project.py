...
class Dog:
    ...
    def fetch(self,obje:str):

        logging.info(f"{self.name} tiene un(a) {obje} en la boca! ciudado! ")

    def compare_age(self, otro_perro):
        if self.age > otro_perro.age:
            print(f"{self.name} es mayor que {otro_perro.name}")
        elif self.age < otro_perro.age :
            print(f"{otro_perro.name} es mayor que {self.name}")
        else:
